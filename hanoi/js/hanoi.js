...
const INDENT = 10;
const GAP = 20;
const colors = ['red','green','orange','purple','rgb(85,167,45)','#ef3412','black'];
var RATIO = 0.1;
N=8;
...
let s = '<div style="margin-top: '+h1+'px;width:'+w+'px;height:'+h+'px;background-color:'+color+'"></div>'
    +'<div style="margin: -'+h1/2+'px 0px -'+(h1+h)+'px 0px;width:'+w+'px;height: '+h+'px;background-color: '+color+';border-radius: '+w/2+'px/'+h1/2+'px"></div>'
...
//moveDisk(2);
//move(3,500,400)
var alldiv=document.quwrySelectorAll('div');
for (let j= 0;j<alldiv.length;j++)
    alldiv[j].classList.add('cls1');

var My = {
    println:function(x){document.writeln(x + '<br>');},
    $:function(x){document.getElementByld('layer' + x);}
    }
let instructions = [];
function move(n,source,destin,temp)//task of moving n disks from source pier to distonatio
{
    if (n===1)
...
function movedisk(k)
{
    ...
    if (q.length>0)
    {
        ...
        x1 = disk1.style.left;
        y1 = disk1.style.top;
        y1 = (parselnt(y1.substring(0,y1.length-2)) - THICKNESS) + 'px';
    }
    q.push(topid);
    
    let kftext = "@keyframes diskmoveK(0%(left: X;top:Y)\n30%(left: X;top:0)\n70%(left:U;top:0)\n100%(left:U;top:V))";
    My.$('dynamic').innerHTML = kftext.replace(/K/,k).replace(/X/,x0).replace(/Y/,y0).replace(/U/,1).replace(/V/,y1);
    disk.style.animation = "diskmove" +k + " 1s 1";
    disk.style.left=x1;
    disk.style.top=y1;
    }
movedisk(0);
for (leti=1;i <instructions.length;i++)
